...
class CGTM_Collect_Data:
    ...
    def check_states(data_frm, possible_states,fl_comp=None):
        '''
...
def build_ternary_saturation_states(self):
    ...
    def cat_states():
        ...
        up_leaf = glob.glob("/home/liam/UDel/resources/test_vor/data/StatesU*New2")
        lo_leaf = glob.glob("/home/liam/UDel/resources/test_vor/data/StatesL*New2")
        ...
        for frm in [0,100,105]:
            for i in range(0,8):
                for j in range(0,8):
                    ...
                    try:
                        tmp_u = pd.read_csv("%sU_%i_%i%iChainsT.csv"%(fl,frm,i,j),index_col=0,header=0)
                    except:
                        continue
                    if tmp_u.values[int(len(tmp_u)*3/4):].sum() == 0:
                        ...
                        tmp_l =  pd.read_csv("%sL_%i_%i%sChainsT.csv"%(fl,frm,i,j),index_col=0,header=0)
                        SU[ind] = tmp_u
                        SL[ind] = tmp_l
                        ind += 1
                        flInd.append("%i%i_%i"%(i,j,frm))
        SU.columns, SL.columns = flInd, flInd
        SU.to_csv("data/SUCH.csv",columns=flInd)
        SL.to_csv("data/SLCH.csv",columns=flInd)
        S = pd.concat([SU,SL],axis=1)
        S.to_csv('data/StatesCH2.csv')
